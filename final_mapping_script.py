...
MIC10_MSA = "Mic10_mafft_fasta.txt"
MIC13_MSA = "Mic13_mafft_fasta.txt"
MIC60_MSA = "Mic60_mafft_fasta.txt"
MIC19_MSA = "Mic19_mafft_fasta.txt"
MIC25_MSA = "Mic25_mafft_fasta.txt"
...
data = pd.read_csv(input_file, index_col = None, header = None)

# here is the first function which would be read by python
def file_parsing(data):
    total_restraints = pd.DataFrame()

    if len(data.columns) == 5: #this shows it's a crosslink file, with query species name written in the name of the file and we are giving it as a sys.argv
        ...
        for i in range(len(data)):
            ...
            if len(new) == 4: # to make sure both proteins are "our protein of interest"
                ...
                if new[1] != 0 and new[3] != 0: # to exclude xlinks mapped to a gap (in which case it will print 0)
                    ...
                    total_restraints = pd.DataFrame(final, columns = ['Protein1', 'Residue1', 'Protein2', 'Residue2','Xl_type'])
