...
def map_to_MSA(protein, query_species, target_species):
    ...
    if protein == 'MIC10':
        ...
        if query_species == 'YEAST':
            protein_query = mapping_MSA["MIC12_query"]
            protein_target = mapping_MSA["MIC13_target"]
        elif target_species == 'YEAST':
            protein_query = mapping_MSA["MIC13_query"]
            protein_target = mapping_MSA["MIC12_target"]
        else:
            protein_query = mapping_MSA["MIC13_query"]
            protein_target = mapping_MSA["MIC13_target"]
...
def converting_to_list(species_name, target_protein, protein_name): #converting the query and target sequences to list
    _list = []
    count = 0
    i = 0

    for record in parse(protein_name, "fasta"):
        if species_name in record.id and target_protein in record.id:
...
def file_parsing(data):
        ...
        for i in range(len(data)):
            ...
            for j in [0,2]: # reading one protein and its residue at a time
              ...
              if protein1 != 'MIC27' and protein1 != 'MIC26': # we are not interested in these proteins
                  list_seq = map_to_MSA(protein1, query_species, target_species)# going to this function to get two aligned seq as lists: query and target
    ...
    elif len(data.columns) == 7: #this shows pairwise file or single protein
        print("this is pairwise biochemical data file")
        final = []
            ...
            if new1[0] == target_species: # for the restraint in target_species only, write that as it is to the output file
                ...
                for j in [1,4]:
                    ...
                    if protein1 != 'MIC27' and protein1 != 'MIC26':
                        list_seq = map_to_MSA(protein1, query_species, target_species)
...
mapping_MSA = {"MIC10_query" :  converting_to_list(query_species, 'MIC10', MIC10_MSA),"MIC10_target" : converting_to_list(target_species,'MIC10', MIC10_MSA),
               "MIC13_query" : converting_to_list(query_species,'MIC13', MIC13_MSA),"MIC13_target" : converting_to_list(target_species,'MIC13', MIC13_MSA),
               "MIC19_query" : converting_to_list(query_species, 'MIC19',MIC19_MSA),"MIC19_target" : converting_to_list(target_species, 'MIC19',MIC19_MSA),
               "MIC25_query" : converting_to_list(query_species,'MIC25', MIC25_MSA),"MIC25_target" : converting_to_list(target_species, 'MIC19', MIC25_MSA),
               "MIC60_query" : converting_to_list(query_species, 'MIC60',MIC60_MSA),"MIC60_target" : converting_to_list(target_species, 'MIC60',MIC60_MSA),
               "MIC12_query" : converting_to_list(query_species,'MIC12', MIC13_MSA),"MIC12_target" : converting_to_list(target_species,'MIC12', MIC13_MSA),
               }
