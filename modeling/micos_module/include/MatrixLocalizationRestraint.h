/* Restraint to calculate a Harmonic restraint for particles if they move into the outer cylinder */

#ifndef IMPMICOS_MATRIX_LOCALIZATION_RESTRAINT_H  // Header guard
#define IMPMICOS_MATRIX_LOCALIZATION_RESTRAINT_H
...
class IMPMICOSEXPORT MatrixLocalizationRestraint : public IMP::Restraint {
    IMP::ParticlesTemp plist_; // All the particles to which to apply this to (individual scores are summed)
    double Rsq_;  // outer radius squared
    double sigma_;  // The sigma for the harmonic


    public:
        MatrixLocalizationRestraint(IMP::ParticlesTemp plist, double R,
        double sigma);
