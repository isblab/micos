...
def compute_dmaps():
    ...
    done_prot_pairs = []
    for p1 in reversed(all_proteins):
        for p2 in tqdm(reversed(all_proteins), desc=f"Processing interactions of {p1}"):
            if ((p1, p2) in done_prot_pairs) or ((p2, p1) in done_prot_pairs) or p1[0:5]==p2[0:5]:
           # if ((p1, p2) in done_prot_pairs) or ((p2, p1) in done_prot_pairs) and 'MIC10' not in p1[0:5] and 'MIC10'not in p2[0:5]:
